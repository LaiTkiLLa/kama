...
export class OrdersStatisticsService {
  ...
  async getStatistics(params: GetOrderStatisticsDto): Promise<OrderStatistics> {
    const { dateFrom, dateTo, marketplaceId, warehouseId } = params;
    ...
    if (marketplaceId !== undefined) {
      query.andWhere('orders.marketplaceId = :marketplaceId', { marketplaceId });
    }

    if (warehouseId !== undefined) {
      query.andWhere('orders.warehouseId = :warehouseId', { warehouseId });
    }
