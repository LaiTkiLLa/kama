...
- Marketplace-independent на `items`: article, логистика/себестоимость/classification, `wbCreatedAt`, **`isArchive`**, карточка товара (`descriptionRussian` / `descriptionEnglish`, `country`, `certificationRequired` / `certificationLink`, `material`, `packagingType` — `1789470000000`).
...
| **In-app LLM agent** | spike: `POST /api/ai/chat`, DeepSeek (`DEEPSEEK_API_KEY`), tools `get_order_statistics` / `get_order_statistics_by_marketplace` / `compare_order_periods` (два периода одним запросом, разницу считает LLM) → `orders_v2`; `get_current_stocks` → агрегат снимка `stocks` «сегодня» (`StocksStatisticsService`; `article` опционален; totals + `byMarketplace`); write-tool `create_test_item` (2026-09-12, расширен 2026-09-22 / 2026-09-23) → `ItemsAiToolsService.createTestItem`: расчётный товар (`created_for_calculation`) + габариты штуки (`*Item` → mp `dimensions`/`volume` + `items_suppliers.dimensions_fact`) + мастер-короб (`*MasterBox` → только `items_suppliers.dimensions_master_box`); `volume` по `*Item` (WB ceil / Ozon факт) → mp + `items_suppliers.volume`; категория, опц. `title`, себестоимость (`costInYuan`/`costInYuanWhite`), `costCalculationType`/`calculationType`/`downloadCalculationMethod` на `items`, `multiplicity` (`numericString`) на supplier-link; write-tools на реальные данные — по-прежнему без плана нельзя. Auth / лимиты / multi-turn — backlog. См. [`ai/in-app-agent.md`](ai/in-app-agent.md). **Не путать** с [`ai/agent-guide.md`](ai/agent-guide.md) (Cursor). |
| **RAG (база знаний агента)** | spike индексации (2026-09-18): `RagModule` (`src/rag`), `POST /api/rag/index` → `docs/rag/*.md` (операторские доки Sheets) → чанки по заголовкам → HF `paraphrase-multilingual-MiniLM-L12-v2` (384) → Qdrant `documentation`. Env `HUGGING_FACE_TOKEN`, `QDRANT_URL`, опц. `QDRANT_COLLECTION` (`qdrantUrl` / `qdrantCollection`); **старт приложения от Qdrant не зависит** (lazy init, DECISION 2026-09-18). Retrieval ✔: tool `search_documentation` → `RagService.searchDocumentation` — two-stage (2026-09-21): Qdrant 10 кандидатов (`score_threshold 0.5`) → `RerankerService` (HF cross-encoder `BAAI/bge-reranker-v2-m3`) → top-3 по `rerankScore`; логи `Qdrant`/`Reranked`. **Reranker не проверен на реальном запуске** (формат входа — склейка `query\ncontent`, доступность модели на `hf-inference`), fallback на порядок Qdrant нет. Payload чанка + `section` (тема документа) и `keywords` (не заполняется). `RagModule` в `AiModule`. Чанкер: breadcrumb + лимит 500 симв.; переиндексация чистит stale-чанки (`deleteBySources`). **Qdrant — внешний docker вне репо** (в compose не добавлять); `Dockerfile` `node:22`. `docs/rag/` — корпус для операторов, dev-docs туда не класть. См. [`ai/rag.md`](ai/rag.md) |
...
Drop `marketplace_id` на `stocks`/`orders_v2`; force cutover без миграции; возврат stocks by-date/v1; секреты в репо; drop/rename `items.isArchive` без явного решения. RAG: смена embedding-модели (= размерность вектора = пересоздание коллекции Qdrant); dev-docs в `docs/rag/` (папка индексируется целиком); возврат зависимости старта приложения от Qdrant (`OnModuleInit` / throw в конструкторе `QdrantService`); Qdrant в `docker-compose.yml` (внешний сервис).
...
| Дата | Итог |
|------|------|
| 2026-09-23 | AI tool `create_test_item`: обязательные `*Item` (штучный товар) → mp `dimensions`/`volume` + `items_suppliers.dimensions_fact`; `*MasterBox` → только `items_suppliers.dimensions_master_box`; `volume` считается по `*Item`. Legacy `POST /api/items` не трогали. См. [`ai/in-app-agent.md`](ai/in-app-agent.md) |
| 2026-09-22 | AI tool `create_test_item` расширен: габариты → `*MasterBox` (мастер-короб); обязательны себестоимость (`costInYuan`/`costInYuanWhite` → `items_suppliers`), `costCalculationType`/`calculationType`/`downloadCalculationMethod` → `items`, `multiplicity` (`numericString`) → supplier-link; `title` опционален (fallback `тестовое название <id>` на item + listings); `dimensionsMasterBox`/`volume` на supplier-link. Legacy `POST /api/items` не трогали. См. [`ai/in-app-agent.md`](ai/in-app-agent.md) |
| 2026-09-22 | Карточка товара на `items` (`1789470000000`): описания RU/EN, страна, сертификация (`certification_required` default false + `certification_link`), материал, тип упаковки. ERP `GET`/`PATCH /api/items/erp/list` (identity через `mpItemId`). `certification_expiration_date` не вводили. Directory эти поля не читает/не пишет. |
| 2026-09-21 | RAG reranker: `RerankerService` (`src/rag/reranker.service.ts`, HF `BAAI/bge-reranker-v2-m3` через `textClassification` + sigmoid, параллельно по кандидатам); `RagService.searchDocumentation` — Qdrant 10 кандидатов → rerank → top-3 по `rerankScore`, два лога. Типы `RerankedChunk`, `DocumentChunk.section` (= первый `#` документа, в payload Qdrant) и `DocumentChunk.keywords` (пока не заполняется). System prompt агента: не раскрывать процесс поиска / chunks / similarity / reranking; частичный ответ — «известное из документации → чего не хватает». Known: вход reranker'а склеен строкой, модель на `hf-inference` не проверена, нет fallback/порога по `rerankScore`. См. [`ai/rag.md`](ai/rag.md) |
| 2026-09-18 | RAG hardening: `QdrantService` lazy init (приложение стартует без Qdrant) + payload index `source` + `deleteBySources` перед upsert + `score_threshold`; `EmbeddingService` без зависимости от Qdrant (`VECTOR_SIZE`); `RagService` — оркестрация, `TOP_K`/`MIN_SIMILARITY`, лог similarity; `ChunkerService` — breadcrumb + лимит 500 симв.; `POST /rag/index` с `api-key`, ответ `{ documents, chunksCount }`; `AiToolExecutor` try/catch → `{ error }` для LLM (все tools); `configuration.ts` — `qdrantUrl`/`qdrantCollection`, убран `telegramBotToken`; `Dockerfile` `node:22-alpine`. Затем: Qdrant из compose убран (внешний сервис, DECISION); типы RAG → `src/rag/interfaces/`; восстановлен lazy `ensureCollection` в `QdrantService` |
| 2026-09-18 | AI tool `search_documentation` (`SearchDocumentationTool` → `RagService.searchDocumentation` → `EmbeddingService.search` + `QdrantService.search`): семантический поиск по `docs/rag/*.md`, top-3 при `similarity ≥ 0.5`. `AiModule` импортирует `RagModule` (`exports: [RagService]`). System prompt: документация — для правил/процессов, данные — через data-tools. Known: порог не откалиброван; `EmbeddingService` зависит от `QdrantService`; ошибки HF/Qdrant в tool → HTTP 500 |
