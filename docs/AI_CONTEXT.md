...
| **RAG (база знаний агента)** | spike индексации (2026-09-18): `RagModule` (`src/rag`), `POST /api/rag/index` → `docs/rag/*.md` (операторские доки Sheets) → чанки по заголовкам → HF `paraphrase-multilingual-MiniLM-L12-v2` (384) → Qdrant `documentation`. Env `HUGGING_FACE_TOKEN`, `QDRANT_URL`, опц. `QDRANT_COLLECTION` (`qdrantUrl` / `qdrantCollection`); **старт приложения от Qdrant не зависит** (lazy init, DECISION 2026-09-18). Retrieval ✔: tool `search_documentation` → `RagService.searchDocumentation` (top-3, `score_threshold 0.5` — порог не откалиброван, `similarity` в логах); `RagModule` в `AiModule`. Чанкер: breadcrumb + лимит 500 симв.; переиндексация чистит stale-чанки (`deleteBySources`). **Qdrant — внешний docker вне репо** (в compose не добавлять); `Dockerfile` `node:22`. `docs/rag/` — корпус для операторов, dev-docs туда не класть. См. [`ai/rag.md`](ai/rag.md) |
