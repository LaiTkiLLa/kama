...
| **In-app LLM agent** | spike: `POST /api/ai/chat`, DeepSeek (`DEEPSEEK_API_KEY`), tools `get_order_statistics` / `get_order_statistics_by_marketplace` / `compare_order_periods` (два периода одним запросом, разницу считает LLM) → `orders_v2`; `get_current_stocks` → агрегат снимка `stocks` «сегодня» (`StocksStatisticsService`; `article` опционален; totals + `byMarketplace`); write-tool `create_test_item` (2026-09-12, расширен 2026-09-22) → `ItemsAiToolsService.createTestItem`: расчётный товар (`created_for_calculation`) + габариты мастер-короба (`*MasterBox` → mp `dimensions`/`volume` + `items_suppliers.dimensions_master_box`/`volume`), категория, опц. `title`, себестоимость (`costInYuan`/`costInYuanWhite`), `costCalculationType`/`calculationType`/`downloadCalculationMethod` на `items`, `multiplicity` (`numericString`) на supplier-link; write-tools на реальные данные — по-прежнему без плана нельзя. Auth / лимиты / multi-turn — backlog. См. [`ai/in-app-agent.md`](ai/in-app-agent.md). **Не путать** с [`ai/agent-guide.md`](ai/agent-guide.md) (Cursor). |
